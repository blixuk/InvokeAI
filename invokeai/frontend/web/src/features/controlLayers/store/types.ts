...
import {
  ...
  zParameterFluxKVCacheEnable,
  zParameterFluxKVCachePrecision,
  zParameterFluxKVCacheStorage,
...
export const zParamsState = z.object({
  ...
  fluxEnableKVCache: zParameterFluxKVCacheEnable,
  fluxKVCachePrecision: zParameterFluxKVCachePrecision,
  fluxKVCacheStorage: zParameterFluxKVCacheStorage,
  ...
  // Ideogram-specific external options
  ideogramNumInferenceSteps: z.number().min(1).max(150).default(50),
  ideogramMagicPrompt: z.boolean().default(true),
  ...
  pidModel: zParameterVAEModel.nullable(), // Optional: Independent PiD model select
  pidTextEncoder: zModelIdentifierField.nullable(), // Optional: Independent PiD text encoder select
  seedResizeEnabled: z.boolean(),
  seedResizeWidth: zParameterImageDimension,
  seedResizeHeight: zParameterImageDimension,
  dimensions: zDimensionsState,
  mrFlowEnabled: z.boolean(),
  mrFlowLowResSteps: z.number().min(1).max(50),
  mrFlowHighResSteps: z.number().min(1).max(10),
  mrFlowSigma: z.number().min(0.01).max(1.0),
});
...
export const getInitialParamsState = (): ParamsState => ({
  ...
  fluxEnableKVCache: false,
  fluxKVCachePrecision: 'auto',
  fluxKVCacheStorage: 'auto',
  ...
  ideogramNumInferenceSteps: 50,
  ideogramMagicPrompt: true,
  usePiDDecode: false,
  pidDecodeSteps: 4,
  pidDecodeSharpness: 1.0,
  pidDecodeTextEncoder: 'gemma-2-2b-it',
  pidDecodeModelVariant: '4k',
  pidDecodeScale: 2,
  pidModel: null,
  pidTextEncoder: null,
  seedResizeEnabled: false,
  seedResizeWidth: 512,
  seedResizeHeight: 512,
  mrFlowEnabled: false,
  mrFlowLowResSteps: 12,
  mrFlowHighResSteps: 1,
  mrFlowSigma: 0.12,
  dimensions: {
    width: 512,
    height: 512,
