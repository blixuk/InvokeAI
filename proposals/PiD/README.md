...
## News
- 🚀 [June 1, 2026] PiD is now fully integrated into **InvokeAI** with complete parameter panel controls, multi-resolution variant selection, and abliterated/uncensored Gemma 2 support!
...
PiD is fully integrated into the **InvokeAI** generation ecosystem as a high-fidelity alternative to standard VAE decoders!

### Key Integration Highlights

- **Seamless Node integration (`pid_decode`)**: Replaces standard VAE decoders inside the `buildFLUXGraph` generation flow.
- **Support for Abliterated Gemma 2**: We added the abliterated/uncensored `IlyaGusev/gemma-2-2b-it-abliterated` model as a configurable text encoder alternative to the standard `gemma-2-2b-it`.
- **Dynamic VRAM Offloading**: To prevent CUDA Out-of-Memory (OOM) errors when loading heavy LDM models (e.g. Flux Transformer + Qwen3/T5), we implemented automatic offloading using InvokeAI's `ModelCache` context wrapper, freeing up over 12 GB of VRAM before starting the PiD pass.
- **Rich Parameter Panel**: Added fully interactive controls directly under `Advanced > Pixel Diffusion Decoder (PiD)`:
  - **PiD Diffusion Steps**: Slider control (1-20).
  - **Detail Sharpness**: Slider control (0.0-2.0).
  - **Text Encoder Variant**: Dropdown supporting *Standard Gemma 2* and *Abliterated Gemma 2*.
  - **Model Resolution Variant**: Dropdown supporting *2K Resolution* and *4K Resolution*.
  - **Upscale Scale Factor**: Dropdown supporting *1x*, *2x*, *4x*, and *8x* upscaling.
